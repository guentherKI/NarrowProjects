...

{
	...
	// Chat AI Mode: "off" | "all" | "mentions"
	window.chatAIMode = "off";
...
window.addEventListener("load", function () {
	...
	let menuCheckInterval = setInterval(() => {
		const menuContainer = document.evaluate("/html/body/div[2]/div[2]/div", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
		if (menuContainer) {
			clearInterval(menuCheckInterval);
			if (settingsBtn === undefined && window.NarrowSDK.NarrowUI && window.NarrowSDK.NarrowUI.CreateMenu) {
				try {
					...
					console.log("Successfully created menu buttons");
				} catch (e) {
					console.error("Failed to create menu buttons:", e);
				}
			}
		}
	}, 1000);
	...
	// Initialize and load Chat-AI mode setting
	if (SettingsGet("chatAI.mode") === undefined) {
		SettingsSet("chatAI.mode", "off");  // Default to off
	}
	window.chatAIMode = SettingsGet("chatAI.mode") || "off";
	console.log("Loaded Chat AI Mode:", window.chatAIMode); // Debug log
	...
	window.NarrowSDK.SquadNetwork.ReplaceMethods();
        ...
        // Initialize Network Receive Hook
        if (window.NarrowSDK.NetworkReceive && typeof window.NarrowSDK.NetworkReceive.ReplaceMethods === 'function') {
            console.log("[Debug] Initializing Network Receive Hook from background.js");
            window.NarrowSDK.NetworkReceive.ReplaceMethods();
            
            // Add a delay to ensure chat container is loaded
            setTimeout(() => {
                if (window.NarrowSDK.NetworkReceive.setupChatObserver) {
                    console.log("[Debug] Setting up chat observer again");
                    window.NarrowSDK.NetworkReceive.setupChatObserver();
                }
            }, 5000); // 5 second delay
        } else {
            console.error("[Debug] Network Receive Hook not properly loaded");
        }
	...
	.extraBtn .main-menu-button-text {
		color: white;
		text-align: center;
		margin-top: 5px;
		font-size: 14px;
	...
	function StgCallback(div) {
		...
		*/
...

		{
			const settingsGroupHeader = document.createElement("h3");
			settingsGroupHeader.className = "settings-group-header";
			settingsGroupHeader.textContent = "Chat AI";
			settingsListDiv.appendChild(settingsGroupHeader);
		}
		...
		{ // Chat AI mode selector
			...
			const settingsItemText = document.createElement("div");
			settingsItemText.className = "settings-item-text";
			settingsItemText.textContent = "Chat-AI Mode";
			settingsItemDiv.appendChild(settingsItemText);
			...
			const options = [
				{ value: "off", text: "Off" },
				{ value: "all", text: "Reply to all messages" },
				{ value: "mentions", text: "Reply on mentions" }
			];
			...
			try {
				const current = SettingsGet("chatAI.mode");
				selectElement.value = current || "off";
			} catch { selectElement.value = "off"; }

			selectElement.addEventListener("change", function (event) {
				const newMode = event.target.value;
				window.chatAIMode = newMode;  // Update global variable
				SettingsSet("chatAI.mode", newMode);
				console.log("Chat AI Mode changed to:", newMode); // Debug log
				NarrowSDK.Utils.ShowAlert("Chat-AI Einstellung gespeichert.", window.clientName, [{ text: "Okay" }]);
			});
		}
	...
	function onFrame() {
		...
		// Always try to create the buttons if they don't exist
		if (settingsBtn === undefined && window.NarrowSDK.NarrowUI && window.NarrowSDK.NarrowUI.CreateMenu) {
			try {
				settingsBtn = window.NarrowSDK.NarrowUI.CreateMenu("60%", "static/img/menuUI/settings.svg", "Extra", "65454", StgCallback);
				window.NarrowSDK.NarrowUI.CreateMenu("35%", "static/img/menuUI/shop/categoryIcons/quiver.svg", "Keybinds", "45356", KbsCallback);

				// Quick Chat-AI toggle button (cycles: off -> mentions -> all)
				try {
					function getChatAILabel(mode) {
						if (mode === 'mentions') return 'ChatAI: Mentions';
						if (mode === 'all') return 'ChatAI: All';
						return 'ChatAI: Off';
					}

					const currentMode = SettingsGet('chatAI.mode') || 'off';
					const chatAIDiv = window.NarrowSDK.NarrowUI.CreateMenu("35%", "static/img/menuUI/settings.svg", getChatAILabel(currentMode), "99999", function (div) {
						const modes = ['off', 'mentions', 'all'];
						let idx = modes.indexOf(SettingsGet('chatAI.mode') || 'off');
						idx = (idx + 1) % modes.length;
						const newMode = modes[idx];
						SettingsSet('chatAI.mode', newMode);
						const textEl = div.getElementsByClassName('main-menu-button-text')[0];
						if (textEl) textEl.textContent = getChatAILabel(newMode);
						NarrowSDK.Utils.ShowAlert('Chat-AI: ' + newMode, window.clientName, [{ text: 'Okay' }]);
					});
				} catch (e) {
					console.warn('Failed to create Chat-AI quick toggle button', e);
				}
